...
test('logs in and creates the only supported activity template', async ({ page }) => {
  await page.route('**/api/admin/auth/login', (r) =>
    r.fulfill({ json: { id: 'admin', role: 'ADMIN' } }),
  );
  await page.route('**/api/admin/auth/me', (r) => r.fulfill({ json: { csrfToken: 'csrf' } }));
  ...
  await page.getByPlaceholder('活动路径代码').fill('expo');
...
test('shows revision conflicts and locks a running activity', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '保存草稿' }).click();
  await expect(page.getByText('版本已被其他人修改，请刷新后重试')).toBeVisible();
  await expect(page.getByRole('button', { name: '提前结束抽奖' })).toBeVisible();
});
