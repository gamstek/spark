...
export class DingTalkCallbackService {
  ...
  async accept(
    ...
    if (!secretsMatch(secret, process.env.DINGTALK_CALLBACK_SECRET ?? ''))
      throw new UnauthorizedException('CALLBACK_UNAUTHORIZED');
    ...
    try {
      return await this.dataSource.transaction(async (manager) => {
        const existing = await manager.query<{ participation_id: string }[]>(
          `SELECT participation_id FROM webhook_receipt WHERE form_id=$1 AND record_id=$2 FOR UPDATE`,
          [parsed.formId, parsed.recordId],
        );
        if (existing[0]) {
          if (existing[0].participation_id !== parsed.participationId)
            throw new ConflictException('RECORD_PARTICIPATION_CONFLICT');
        ...
        if (!bound[0])
          throw new ConflictException('FORM_PARTICIPATION_MISMATCH');
        const receiptId = randomUUID();
        await manager.query(
          `INSERT INTO webhook_receipt (id,form_id,record_id,participation_id,payload) VALUES ($1,$2,$3,$4,$5)`,
          [
            receiptId,
            parsed.formId,
            parsed.recordId,
        ...
        await this.jobs.enqueue(
          manager,
          'DINGTALK_SUBMISSION',
          `dingtalk:${parsed.formId}:${parsed.recordId}`,
