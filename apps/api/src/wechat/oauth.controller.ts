...
function secureSuffix(): string { return process.env.NODE_ENV === 'production' ? '; Secure' : ''; }
...
export class OAuthController {
  ...
  @Get('start')
  async start(@Query('returnPath') returnPath: string, @Res() reply: FastifyReply) {
    const issued = await this.states.issue(returnPath);
    reply.header('Set-Cookie', `spark_oauth_nonce=${issued.browserNonce}; Path=/api/wechat/oauth; HttpOnly; SameSite=Lax; Max-Age=600${secureSuffix()}`);
    ...
    authorize.search = new URLSearchParams({ appid: process.env.WECHAT_APP_ID ?? '', redirect_uri: callback, response_type: 'code', scope: 'snsapi_base', state: issued.state }).toString();
    return reply.redirect(`${authorize.toString()}#wechat_redirect`);
  }

  @Get('callback')
  async callback(@Query('code') code: string, @Query('state') state: string, @Req() request: FastifyRequest, @Res() reply: FastifyReply) {
    ...
    reply.header('Set-Cookie', `${cookieNames.ACTIVITY}=${session.token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${7 * 24 * 60 * 60}${secureSuffix()}`);
