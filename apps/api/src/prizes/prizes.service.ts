...
export class PrizesService {
  ...
  async create(
    ...
    await this.dataSource.transaction(async (manager) => {
      await manager.query(`INSERT INTO prize (id,name) VALUES ($1,$2)`, [prizeId, input.name]);
      await manager.query(
        `INSERT INTO activity_prize (id,activity_id,prize_id,total_stock,weight,prize_name) VALUES ($1,$2,$3,$4,$5,$6)`,
        [id, activityId, prizeId, input.totalStock, input.weight, input.name],
      );
    });
    return { id };
  }
  async addStock(
    ...
    await this.dataSource.transaction(async (manager) => {
      await manager.query(`SELECT id FROM activity_prize WHERE id=$1 FOR UPDATE`, [
        activityPrizeId,
      ]);
      ...
      await manager.query(`UPDATE activity_prize SET total_stock=total_stock+$2 WHERE id=$1`, [
        activityPrizeId,
        quantity,
      ]);
      await manager.query(
