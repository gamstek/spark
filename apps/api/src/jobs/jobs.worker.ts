...
export interface ClaimedJob { id: string; kind: string; payload: unknown; attempts: number }
...
function sanitizeError(error: unknown): string {
  ...
  return message.replace(/\b1\d{10}\b/g, '[redacted-phone]').replace(/[\w.+-]+@[\w.-]+/g, '[redacted-email]').slice(0, 200);
}
...
export class JobsWorker {
  ...
  constructor(
    ...
    @Optional() @Inject(JobHandlers) private readonly sharedHandlers?: JobHandlers,
  ) {
    ...
    return this.dataSource.transaction(async (manager) => {
      const result = await manager.query<[ClaimedJob[], number]>(
        `WITH candidate AS (
         ...
         RETURNING job.id, job.kind, job.payload, job.attempts`, [now, this.owner],
      );
  ...
  async complete(id: string): Promise<boolean> {
    const result = await this.dataSource.query(
      `UPDATE background_job SET status='SUCCEEDED', lease_owner=NULL, lease_until=NULL, updated_at=now()
       WHERE id=$1 AND status='RUNNING' AND lease_owner=$2`, [id, this.owner],
    );
    return writeCount(result) === 1;
  }

  private async fail(job: ClaimedJob, error: unknown, now: Date): Promise<void> {
    const terminal = job.attempts >= 5;
    const delaySeconds = delays[Math.min(job.attempts - 1, delays.length - 1)] ?? 600;
    await this.dataSource.query(
      ...
      [job.id, now, terminal ? 'FAILED' : 'PENDING', new Date(now.getTime() + delaySeconds * 1000), sanitizeError(error), this.owner],
    );
  }
  ...
  async runDueJobs(now: Date): Promise<number> {
    ...
    while (true) {
      ...
      const handler = this.localHandlers.get(job.kind) ?? this.sharedHandlers?.get(job.kind);
      try {
