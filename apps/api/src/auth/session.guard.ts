...
export const RequireSession = (role: SessionRole) =>
  SetMetadata(SESSION_ROLE, role);
export const cookieNames = {
  ACTIVITY: 'spark_activity',
  STAFF: 'spark_staff',
  ADMIN: 'spark_admin',
} as const;
...
export class SessionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const role = this.reflector.getAllAndOverride<SessionRole>(SESSION_ROLE, [
      context.getHandler(),
      context.getClass(),
    ]);
    if (!role) return true;
