...
describe('durable PostgreSQL jobs', () => {
  ...
  it('rolls back an enqueued job with its caller transaction', async () => {
    await expect(database.dataSource.transaction(async (manager) => {
      await jobs.enqueue(manager, 'ROLLBACK', 'rollback-key', { value: 1 });
      throw new Error('rollback');
    })).rejects.toThrow('rollback');
    const rows = await database.dataSource.query<{ count: string }[]>(`SELECT count(*) FROM background_job WHERE deduplication_key='rollback-key'`);
  ...
  it('deduplicates enqueue by key', async () => {
    ...
    const rows = await database.dataSource.query<{ count: string }[]>(`SELECT count(*) FROM background_job WHERE deduplication_key='same-key'`);
    expect(Number(rows[0]?.count)).toBe(1);
  });

  it('lets only one concurrent worker claim a job', async () => {
    await database.dataSource.query(`UPDATE background_job SET status='SUCCEEDED' WHERE deduplication_key='same-key'`);
    await database.dataSource.transaction((manager) => jobs.enqueue(manager, 'CONCURRENT', 'concurrent-key', {}));
    const workerA = new JobsWorker(database.dataSource, 'worker-a');
    const workerB = new JobsWorker(database.dataSource, 'worker-b');
    const [a, b] = await Promise.all([workerA.claimDueJob(new Date()), workerB.claimDueJob(new Date())]);
    expect([a, b].filter(Boolean)).toHaveLength(1);
  });

  it('recovers an expired lease and rejects completion by the old owner', async () => {
    await database.dataSource.transaction((manager) => jobs.enqueue(manager, 'RECOVER', 'recover-key', {}));
    const oldWorker = new JobsWorker(database.dataSource, 'old-worker');
    const claimed = await oldWorker.claimDueJob(new Date());
    expect(claimed).not.toBeNull();
    await database.dataSource.query(`UPDATE background_job SET lease_until=now() - interval '1 second' WHERE id=$1`, [claimed?.id]);
  ...
  it('retries with backoff, stops after five attempts, and can replay original payload', async () => {
    await database.dataSource.transaction((manager) => jobs.enqueue(manager, 'FAIL', 'failure-key', { safe: 'original' }));
    const worker = new JobsWorker(database.dataSource, 'failure-worker');
    worker.register('FAIL', async () => { throw new Error('phone 13800138000 failed'); });
    let now = new Date();
    ...
    const replayed = await database.dataSource.query<{ payload: { safe: string }; attempts: number }[]>(`SELECT payload, attempts FROM background_job WHERE id=$1`, [job!.id]);
    expect(replayed[0]).toMatchObject({ payload: { safe: 'original' }, attempts: 0 });
  });
});
