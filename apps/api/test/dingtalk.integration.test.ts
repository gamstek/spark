...
const config = {
  formId: 'form-test',
  formUrl: 'https://alidocs.dingtalk.com/notable/share/form/test?participant=',
...
describe('DingTalk form integration', () => {
  ...
  let callbacks: DingTalkCallbackService;
  let handlers: JobHandlers;
  beforeAll(async () => {
    database = await createTestDatabase();
    process.env.DINGTALK_CALLBACK_SECRET = 'test-shared-secret';
  });
  beforeEach(async () => {
    ...
    await database.dataSource.query(
      `UPDATE activity_version SET config=$1 WHERE id=(SELECT published_version_id FROM activity WHERE id=$2)`,
      [config, scenario.activityId],
    );
    await database.dataSource.query(
      `UPDATE activity_participation SET lead_completed=false,lead_completed_at=NULL WHERE activity_id=$1`,
      [scenario.activityId],
    );
    handlers = new JobHandlers();
    callbacks = new DingTalkCallbackService(
      database.dataSource,
  ...
  afterAll(async () => {
    delete process.env.DINGTALK_CALLBACK_SECRET;
  ...
  const input = (
    ...
    formId,
    recordId,
    participationId: participationId ?? scenario.participationIds[0],
  ...
  it('adds or replaces the configured prefill field', async () => {
    const service = new DingTalkPrefillService(
      ...
      { get: async () => ({ nextStep: 'FORM' }) } as never,
    );
    const result = await service.createFormUrl(
      scenario.userIds[0],
      'expo-2026',
    );
    expect(new URL(result.url).searchParams.get('participant')).toBe(
  ...
  it('rejects invalid secrets, malicious ids and cross-form submissions', async () => {
    await expect(callbacks.accept(input(), 'wrong')).rejects.toThrow(
      'CALLBACK_UNAUTHORIZED',
    );
    await expect(
      callbacks.accept(
        { ...input(), participationId: "' OR true --" },
        'test-shared-secret',
      ),
    ).rejects.toThrow('CALLBACK_INVALID');
    await expect(
      callbacks.accept(
        input('record-x', undefined, 'another-form'),
        'test-shared-secret',
      ),
    ).rejects.toThrow('FORM_PARTICIPATION_MISMATCH');
    expect(
      await database.dataSource.query(`SELECT id FROM webhook_receipt`),
    ).toHaveLength(0);
  });

  it('reliably accepts once and does not grant eligibility before the worker runs', async () => {
    await expect(
      callbacks.accept(input(), 'test-shared-secret'),
    ...
    expect(
      await database.dataSource.query(`SELECT id FROM background_job`),
    ).toHaveLength(1);
  });
  ...
  it('returns conflict when a record id is replayed for another participant', async () => {
    ...
    await expect(
      callbacks.accept(
        input('record-1', scenario.participationIds[1]),
        'test-shared-secret',
      ),
    ).rejects.toThrow('RECORD_PARTICIPATION_CONFLICT');
  });
  ...
  it('processes submissions once, retains later records and adopts the earliest lead', async () => {
    ...
    const submissions = await database.dataSource.query<
      { record_id: string }[]
    >(
      `SELECT record_id FROM dingtalk_form_submission ORDER BY submitted_at,record_id`,
    );
    const participation = await database.dataSource.query<
      { lead_completed_at: Date | null; adopted_submission_id: string | null }[]
    >(
      `SELECT lead_completed_at,adopted_submission_id FROM activity_participation WHERE id=$1`,
      [scenario.participationIds[0]],
    );
    expect(submissions.map((row) => row.record_id)).toEqual([
      'record-1',
      'record-2',
    ]);
    ...
    expect(
      await database.dataSource.query(
        `SELECT id FROM lottery_record WHERE activity_id=$1 AND user_id=$2`,
  ...
  it('can be resent after persistence failure and still records leads after the activity ends', async () => {
    ...
    let fail = true;
    database.dataSource.transaction = ((...args: unknown[]) =>
      fail
        ? ((fail = false), Promise.reject(new Error('database unavailable')))
        : original(
            ...(args as Parameters<typeof original>),
          )) as typeof database.dataSource.transaction;
    await expect(
      callbacks.accept(input(), 'test-shared-secret'),
    ).rejects.toThrow('CALLBACK_NOT_PERSISTED');
    await callbacks.accept(input(), 'test-shared-secret');
    database.dataSource.transaction = original;
    await database.dataSource.query(
      `UPDATE activity_version SET starts_at=$2,draw_ends_at=$3,ends_at=$4,redeem_ends_at=$4 WHERE id=(SELECT published_version_id FROM activity WHERE id=$1)`,
      [
        scenario.activityId,
    ...
    await new JobsWorker(
      database.dataSource,
      'ended-test',
      handlers,
    ).runDueJobs(new Date());
    const row = await database.dataSource.query<{ lead_completed: boolean }[]>(
      `SELECT lead_completed FROM activity_participation WHERE id=$1`,
      [scenario.participationIds[0]],
    );
