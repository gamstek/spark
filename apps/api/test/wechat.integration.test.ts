...
describe('WeChat identity and participation recovery', () => {
  ...
  it('consumes an OAuth state only once and binds it to a browser nonce', async () => {
    const states = new OAuthStateService(database.dataSource, 'state-secret', () => scenario.now);
    const issued = await states.issue('/activity/expo-2026?channel=poster');
    await expect(states.consume(issued.state, issued.browserNonce)).resolves.toBe('/activity/expo-2026?channel=poster');
    await expect(states.consume(issued.state, issued.browserNonce)).rejects.toThrow('OAUTH_STATE_INVALID');

    const another = await states.issue('/activity/expo-2026');
    await expect(states.consume(another.state, 'different-browser')).rejects.toThrow('OAUTH_STATE_INVALID');
  });

  it('rejects expired states and off-site return paths', async () => {
    let clock = scenario.now;
    const states = new OAuthStateService(database.dataSource, 'state-secret', () => clock);
    expect(() => states.validateReturnPath('https://evil.example/activity/x')).toThrow('RETURN_PATH_INVALID');
    expect(() => states.validateReturnPath('//evil.example/activity/x')).toThrow('RETURN_PATH_INVALID');
  ...
  it('maps concurrent first login for one OpenID to one user', async () => {
    const identities = new WechatIdentityService(database.dataSource, 'wx-app-id');
    const results = await Promise.all(Array.from({ length: 5 }, () => identities.getOrCreateUser('openid-concurrent')));
  ...
  it('creates one participation per user and activity under concurrency', async () => {
    ...
    await database.dataSource.query(`INSERT INTO user_account (id) VALUES ($1)`, [userId]);
    const results = await Promise.all(Array.from({ length: 5 }, () => participants.getOrCreate(userId, scenario.activityId)));
  ...
  it('surfaces WeChat timeouts for retry instead of inventing identity', async () => {
    const gateway = new WechatGateway({ appId: 'app', appSecret: 'secret', fetch: async () => { throw new Error('timeout'); } });
    await expect(gateway.exchangeCode('code')).rejects.toThrow('WECHAT_REQUEST_FAILED');
  });
  ...
  it('shares an encrypted access token and caches subscription for at most 60 seconds', async () => {
    ...
    try {
      await new WechatIdentityService(database.dataSource, 'wx-cache-app').getOrCreateUser('openid-cache');
      let tokenCalls = 0;
      let subscriptionCalls = 0;
      const gateway = {
        async fetchStableAccessToken() { tokenCalls += 1; return { accessToken: 'remote-token', expiresIn: 7200 }; },
        async isSubscribed() { subscriptionCalls += 1; return true; },
      } as unknown as WechatGateway;
      const tokens = new WechatTokenService(database.dataSource, gateway);
      const subscriptions = new SubscriptionService(database.dataSource, gateway, tokens);
      await expect(subscriptions.isSubscribed('openid-cache')).resolves.toBe(true);
      await expect(subscriptions.isSubscribed('openid-cache')).resolves.toBe(true);
      expect(tokenCalls).toBe(1);
      expect(subscriptionCalls).toBe(1);
      const stored = await database.dataSource.query<{ access_token_ciphertext: string }[]>(`SELECT access_token_ciphertext FROM wechat_credential_cache WHERE app_id='wx-cache-app'`);
