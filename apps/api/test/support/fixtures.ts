...
export async function createScenario(dataSource: DataSource, options: { now?: Date } = {}): Promise<Scenario> {
  ...
  await dataSource.transaction(async (manager) => {
    await manager.query(`INSERT INTO admin_account (id, username, password_hash, display_name) VALUES ($1, 'admin-fixture', 'test-only-hash', '测试管理员')`, [adminId]);
    await manager.query(`INSERT INTO staff_account (id, username, password_hash, display_name) VALUES ($1, 'staff-fixture', 'test-only-hash', '测试工作人员')`, [staffId]);
