...
export async function createScenario(
  ...
  options: { now?: Date; accountPassword?: string } = {},
): Promise<Scenario> {
  ...
  const redeemCodeHash = createHash('sha256')
    .update('fixture-redeem-code')
    .digest('hex');
  const passwordHash = await hashPassword(
    options.accountPassword ?? 'fixture-password-only',
  );

  await dataSource.transaction(async (manager) => {
    await manager.query(
      `INSERT INTO admin_account (id, username, password_hash, display_name) VALUES ($1, 'admin-fixture', $2, '测试管理员')`,
      [adminId, passwordHash],
    );
    await manager.query(
      `INSERT INTO staff_account (id, username, password_hash, display_name) VALUES ($1, 'staff-fixture', $2, '测试工作人员')`,
    ...
    await manager.query(
      `INSERT INTO activity (id, code, name) VALUES ($1, 'expo-2026', '展会抽奖')`,
      [activityId],
    );
    await manager.query(
      `INSERT INTO activity_version (id, activity_id, version, status, template_id, template_version, config_schema_version, config, starts_at, ends_at, draw_ends_at, redeem_ends_at, published_at, activity_name) VALUES ($1,$2,1,'PUBLISHED','exhibition-lottery',1,1,'{}',$3,$4,$4,$5,$3,'Fixture activity')`,
      [
        versionId,
        activityId,
        now,
    ...
    for (let i = 0; i < userIds.length; i += 1)
      await manager.query(
        `INSERT INTO activity_participation (id, activity_id, user_id, lead_completed, lead_completed_at) VALUES ($1,$2,$3,true,$4)`,
        [participationIds[i], activityId, userIds[i], now],
      );
    await manager.query(`INSERT INTO prize (id, name) VALUES ($1, '一等奖')`, [
      prizeId,
    ]);
    await manager.query(
    ...
    await manager.query(
      `INSERT INTO redemption (id, lottery_record_id, redeem_code_hash, status, redeem_end_at) VALUES ($1,$2,$3,'WAIT_REDEEM',$4)`,
      [
        redemptionId,
        lotteryRecordId,
        redeemCodeHash,
