...
describe('redemption recovery and atomic confirmation', () => {
  ...
  beforeEach(async () => {
    ...
    code = codes.restore(encrypted.encryptedCode, encrypted.keyId);
    await database.dataSource.query(
      `UPDATE lottery_record SET prize_name='一等奖',prize_image_url='/media/prize.png',redeem_end_at=$2 WHERE id=$1`,
      [scenario.lotteryRecordId, new Date(scenario.now.getTime() + 86_400_000)],
    );
    await database.dataSource.query(
      `UPDATE redemption SET redeem_code_hash=$2,encrypted_code=$3,encryption_key_id=$4,redeem_end_at=$5 WHERE lottery_record_id=$1`,
      [
    ...
    await database.dataSource.query(
      `INSERT INTO staff_account (id,username,password_hash,display_name) VALUES ($1,'staff-two',$2,'工作人员二')`,
      [secondStaffId, await hashPassword('test-only-password')],
    );
    await database.dataSource.query(
      `INSERT INTO staff_activity_permission (staff_account_id,activity_id) VALUES ($1,$2)`,
      [secondStaffId, scenario.activityId],
    );
  });
  afterAll(async () => {
  ...
  const service = (now = scenario.now) =>
    new RedemptionsService(database.dataSource, codes, () => now);
  ...
  it('restores the same code for its owning user and rejects other users', async () => {
    ...
    expect(first.qrUrl).toContain(encodeURIComponent(code));
    await expect(
      service().getOwnCode(scenario.userIds[1], 'expo-2026'),
    ).rejects.toThrow('PRIZE_CODE_NOT_FOUND');
  });
  ...
  it('lookup does not redeem and exposes only a masked identity hint', async () => {
    ...
    expect(result.status).toBe('WAIT_REDEEM');
    expect(result.userHint).toMatch(/^用户…[\da-f]{4}$/);
    expect(
      (
        await database.dataSource.query<{ redeemed_at: Date | null }[]>(
          `SELECT redeemed_at FROM redemption`,
        )
      )[0]?.redeemed_at,
    ).toBeNull();
  });
  ...
  it('allows two staff confirmations to create one transition and audit event', async () => {
    const [first, second] = await Promise.all([
      service().confirm(code, scenario.staffId),
      service().confirm(code, secondStaffId),
    ]);
  ...
  it('rejects missing permissions without revealing the redemption', async () => {
    ...
    await database.dataSource.query(
      `INSERT INTO staff_account (id,username,password_hash,display_name) VALUES ($1,'no-access',$2,'无权限')`,
      [unauthorizedStaff, await hashPassword('test-only-password')],
    );
    await expect(service().lookup(code, unauthorizedStaff)).rejects.toThrow(
      'REDEMPTION_NOT_FOUND',
    );
    await expect(service().confirm(code, unauthorizedStaff)).rejects.toThrow(
      'REDEMPTION_NOT_FOUND',
    );
  });
  ...
  it('rejects the exact redemption deadline but preserves existing redeemed results', async () => {
    ...
    await expect(
      service(deadline).confirm(code, scenario.staffId),
    ).rejects.toThrow('REDEMPTION_EXPIRED');
    expect(
      (
        await database.dataSource.query<{ status: string }[]>(
          `SELECT status FROM redemption`,
        )
      )[0]?.status,
    ).toBe('EXPIRED');
    expect(
      (await service(deadline).lookup(code, scenario.staffId)).status,
    ).toBe('EXPIRED');
    await expect(
      service(deadline).getOwnCode(scenario.userIds[0], 'expo-2026'),
    ).rejects.toThrow('PRIZE_CODE_NOT_AVAILABLE');
  });
  ...
  it('fails closed with a missing or incorrect encryption key', async () => {
    ...
    process.env.REDEEM_CODE_KEYS = JSON.stringify({
      'test-key': Buffer.alloc(32, 8).toString('base64'),
    });
    await expect(
      service().getOwnCode(scenario.userIds[0], 'expo-2026'),
    ).rejects.toThrow();
