...
停止 API 写入后，在现有数据库执行
`pnpm --filter @spark/api db:migrate`。前向迁移会保留活动、用户、奖品及其他无关数据，删除旧外部表单的回调和报名答案，移除旧表单配置键，并创建
`activity_form_submission`。已关联旧答案的参与记录会清除完成状态，用户需要重新填写自有表单；未关联旧答案的参与状态不变。迁移不搬迁旧答案。

随后确认
`ACTIVITY_IDENTITY_MODE=anonymous`，实际提交 13 题表单，并核对参与者详情、XLSX 列、抽奖与核销。表单答案与留资状态在一次请求的同一事务中保存；通用后台任务继续用于导出和维护。删除旧答案不可逆，回滚应用版本不能恢复这些数据。

本次升级跨越数据库 schema
epoch。11000 成功后，迁移前的旧镜像不能自动回滚或重新激活；候选镜像在迁移后失败时，保持服务停止，修复后前向发布兼容镜像，或从升级前备份恢复数据库。部署脚本会比较候选与当前 release 的
`schemaEpoch`，跨越 epoch 时阻止自动镜像回滚。
