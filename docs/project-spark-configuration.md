...
| 配置                    | 具体含义                                                                                  | 从哪里获取                    | 是否保密             |
| ----------------------- | ----------------------------------------------------------------------------------------- | ----------------------------- | -------------------- |
| `WECHAT_APP_ID`         | 当前微信公众号的开发者 ID，通常以 `wx` 开头。它不是公众号原始 ID `gh_xxx`。               | 微信公众号后台的开发/基本配置 | 否，但只配置在服务端 |
| `WECHAT_APP_SECRET`     | 与公众号 AppID 配套的开发密钥，用于获取公众号接口所需的 access token。                    | 微信公众号后台生成或重置      | 是                   |
| `WECHAT_CALLBACK_TOKEN` | 用于校验微信公众号服务器回调签名的 Token；由部署者生成，并在公众号后台填写同一个值。      | 部署配置和公众号后台          | 是                   |
| `PUBLIC_BASE_URL`       | 对外可访问的 HTTPS 根地址，生产固定为 `https://spark.gamstek.com`；活动入口链接以它生成。 | 部署配置                      | 否                   |

微信公众号后台还要把生产服务器的固定出口 IP 加入接口 IP 白名单。生产的
`PUBLIC_BASE_URL` 必须精确设置为：

```text
https://spark.gamstek.com
```

活动入口不使用网页 OAuth；微信公众号服务器回调地址固定为：
...
公众号事件产生的活动入口链接是一位用户的一次性链接，不能复制到海报、群公告或客服话术中作为公共入口。公共物料应引导用户关注公众号，随后通过关注回复或
`LOTTERY`
菜单获得自己的链接。链接在十分钟后失效，首次打开后即被消费；失效、已使用或被截断的链接会回到活动页并显示入口无效。
