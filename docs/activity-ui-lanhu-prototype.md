...
> 实施更新（2026-09-12）：活动页采用自有固定 13 题表单，沿用现有品牌红、字体与表单布局。Q1–Q12 必填、Q13 可选，选中“其他”时显示必填说明；提交前须同意隐私协议。草稿按活动和会话保存，提交成功后进入抽奖。后台提供完整详情和分列导出。下文原型中的旧字段仅供视觉参考，当前字段以共享表单定义为准。初始数据库结构已重写，部署必须从空库重建，不迁移或保留旧报名数据。
